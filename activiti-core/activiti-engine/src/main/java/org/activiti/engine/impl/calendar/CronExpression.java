...
public class CronExpression implements Serializable, Cloneable {
  ...
  protected void buildExpression(String expression) throws ParseException {
    ...
    protected int storeExpressionVals(int pos, String s, int type) throws ParseException {
        ...
        if (Character.isLetter(c) && isValidLetterExpression(s, i)) {
            i = processLetterExpression(s, i, type);
        } else if (c == '?') {
            i = processQuestionMark(s, i, type);
        } else if (c == '*' || c == '/') {
            i = processAsteriskOrSlash(s, i, type);
        } else if (c == 'L') {
            i = processL(s, i, type);
        } else if (Character.isDigit(c)) {
            i = processDigit(s, i, type);
        } else {
    ...
    private boolean isValidLetterExpression(String s, int i) {
        return !s.equals("L") && !s.equals("LW") && !s.matches("^L-[0-9]*[W]?");
    }

    private int processLetterExpression(String s, int i, int type) throws ParseException {
        String sub = s.substring(i, i + 3);
        int sval = -1, eval = -1;

        if (type == MONTH) {
            sval = getMonthNumber(sub) + 1;
            validateMonth(sval, i);
            if (s.length() > i + 3 && s.charAt(i + 3) == '-') {
                i += 4;
                sub = s.substring(i, i + 3);
                eval = getMonthNumber(sub) + 1;
                validateMonth(eval, i);
            }
        } else if (type == DAY_OF_WEEK) {
            sval = getDayOfWeekNumber(sub);
            validateDayOfWeek(sval, i);
            if (s.length() > i + 3) {
                char c = s.charAt(i + 3);
                if (c == '-') {
                    i += 4;
                    ...
                    validateDayOfWeek(eval, i);
                } else if (c == '#') {
                    i += 4;
                    nthdayOfWeek = parseNthDayOfWeek(s, i);
                } else if (c == 'L') {
                    lastdayOfWeek = true;
                    i++;
                }
        ...
        addToSet(sval, eval, eval != -1 ? 1 : 0, type);
        return i + 3;
    }

    private void validateMonth(int month, int i) throws ParseException {
        if (month <= 0) {
            throw new ParseException("Invalid Month value", i);
        }
    }

    private void validateDayOfWeek(int dayOfWeek, int i) throws ParseException {
        if (dayOfWeek < 0) {
            throw new ParseException("Invalid Day-of-Week value", i);
        }
    }

    private int parseNthDayOfWeek(String s, int i) throws ParseException {
        try {
            int nthday = Integer.parseInt(s.substring(i));
    ...
    private int processQuestionMark(String s, int i, int type) throws ParseException {
        i++;
    ...
    private int processAsteriskOrSlash(String s, int i, int type) throws ParseException {
        if (s.charAt(i) == '*' && (i + 1) >= s.length()) {
            addToSet(ALL_SPEC_INT, -1, 0, type);
            return i + 1;
        } else if (s.charAt(i) == '/' && ((i + 1) >= s.length() || s.charAt(i + 1) == ' ' || s.charAt(i + 1) == '\t')) {
            throw new ParseException("'/' must be followed by an integer.", i);
        } else if (s.charAt(i) == '*') {
            i++;
        }
        return processSlash(s, i, type);
    }

    private int processSlash(String s, int i, int type) throws ParseException {
        if (s.charAt(i) == '/') {
            i++;
            if (i >= s.length()) {
                throw new ParseException("Unexpected end of string.", i);
            }
            int incr = getNumericValue(s, i);
            validateIncrement(incr, type, i);
            i += Integer.toString(incr).length();
            addToSet(ALL_SPEC_INT, -1, incr, type);
        } else {
            addToSet(ALL_SPEC_INT, -1, 1, type);
        }
        return i;
    }

    private void validateIncrement(int incr, int type, int i) throws ParseException {
        if (incr > 10) {
            i++;
        }
    ...
    private int processL(String s, int i, int type) throws ParseException {
        i++;
    ...
    private int processDigit(String s, int i, int type) throws ParseException {
